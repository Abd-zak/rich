...
TString rarDatasets::getWeightVarName(TString datasetName)
{
  ...
  if (_fullFObs->find(swvName)) {
    cout << "Will use " << swvName << " as weight for dataset " << datasetName << endl;
  } else {
    cout << "Can not find " << swvName << " as a variable to use as a weight." << endl;
    swvName="";
  }
  return(swvName);
}
...
void rarDatasets::tabulateDatasets(const char *dsName)
{
  ...
  cout << "Tabulate Datsets by Category\n" << endl;
