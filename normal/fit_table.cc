...
int main(int argc, char *argv[]){
  ...
  if (!read_options(optFile)) return 1;    // ***** READING OPTIONS
...
#define DEBUG_PID
  ...
  // - 5 - background   (equal to 1 in old versions of RICH code)
    ...
    for (int i = 0; i<4; i++) {
      for (int j = 0; j<6; j++) {
	printf("lh_cut[%d][%d] = %.2f ",i,j,lh_cut[i][j]);
      }
      printf("\n");
    }
    // =>=>
    // i = 0,1,2,3 = pi,K,pbthr,pathr
    // j = 0,1,2,3,4,5 = pi,K,p,e,mu,bkg
    //                    [01]: LH_pi_K,    [02]: LH_pi_p [05] LH_pi_bg
    // [10]: LH_K_pi,                       [12]: LH_K_p
...
void create_hist(){
  ...
  const char  *tags[6] = {"K0+",   "K0-",   "#phi+", "#phi-", "#Lambda+",  "#Lambda-"};
  ...
  // ***** ARMENTEROS/KINEMATICS (after basic cuts)
  ...
  char hT[] = "#Lambda+ pi 17<P<50 0.00<#theta<.004";
  size_t sT = strlen(hT)+1;
  ...
  for(int i = 0; i<6; i++){      //6
	...
	for(int t = 0; t<Nt; t++){
	  ...
	  snprintf(hT,sT,"%s %s %.0f<P<%.0f %.2f<#theta<%.2f",
		   tags[i],id[j].c_str(),p_bins[p],p_bins[p+1],t_bins[t],t_bins[t+1]);
...
void write_hist(){
  ...
  if(analysis=="iPhi")
    {
      TFile* output = new TFile("hist_iphi.root","RECREATE");
      for(int i = 2; i<4; i++){      //6
	for(int j = 0; j<5; j++){  //4
	  for(int p = 0; p<Np;p++){
	    for(int t = 0; t<Nt; t++){
...
void get_input_data_t1(){
  ...
  if (!tree) {
    printf("** get_input_data_t1: No \"tree\" TTree in TFile \"%s\"\n",
	   gFile->GetName());
    exit(1);
  }
  ...
  for (Long64_t jentry=0; jentry<nentries1;jentry++) {
    ...
    double mLambda = lv_Lambda->Mag(), mK0 = lv_K0->Mag();
    double Pm = lv_pim->Vect().Mag(), Pp = lv_pip->Vect().Mag();
    double P2m = Pm*Pm, P2p = Pp*Pp;
    double mpipi, mppi, mpip;
    double Epipi =  sqrt(P2p+M2_pi )+sqrt(P2m+M2_pi );
    if (Lambda==1) {
      double Eppi = sqrt(P2p+M2_p )+sqrt(P2m+M2_pi);
      mpipi = sqrt(mLambda*mLambda+Epipi*Epipi-Eppi*Eppi);
      mppi =  sqrt(mK0*mK0        -Epipi*Epipi+Eppi*Eppi);
    }
    else {
      double Epip = sqrt(P2p+M2_pi)+sqrt(P2m+M2_p );
      mpipi = sqrt(mLambda*mLambda+Epipi*Epipi-Epip*Epip);
      mpip =  sqrt(mK0*mK0        -Epipi*Epipi+Epip*Epip);
    }
    static int debug = 0;
    if (debug)
      printf("Lambda %.3f %.3f K0 %.3f %.3f\n",
	     mLambda,Lambda==1?mppi:mpip,mK0,mpipi);
#endif
    
    if( pipe || !rpipe){

      am_all->Fill(alpha,pt1);  // ***** ARMENTEROS/KINEMATICS
      Z_all->Fill(vz); XY_all->Fill(vx,vy);
...
void get_input_data_t2(){
  ...
  if (!tree2) {
    printf("** get_input_data_t2: No \"tree2\" TTree in TFile \"%s\"\n",
	   gFile->GetName());
    exit(1);
  }
...
#define DEBUG_IPHI
  ...
  tree2->SetBranchAddress("vx",&vx);
  tree2->SetBranchAddress("vy",&vy);
  tree2->SetBranchAddress("vz",&vz);
...
#define LOOSE_K_SELECTION
	...
	double piKMx = 60; // Max. P for pi/K resolution
	...
	if (lv_kp->Vect().Mag()>pi_thr2) p_bin_m = i; 
#else
	if (lv_kp->Vect().Mag()>k_thr2)  p_bin_m = i;
#endif
      }
      if(lv_kp->Vect().Mag() >= p_bins[i] && lv_kp->Vect().Mag() < p_bins[i+1]){
#ifdef LOOSE_K_SELECTION
	// K+ loose selection: require P- > piThr instead of P- > Kthr
	if (lv_km->Vect().Mag()>pi_thr2) p_bin_p = i; 
#else
	if (lv_km->Vect().Mag()>k_thr2)  p_bin_p = i;
#endif
      }
    }
    for(int i = 0 ; i<Nt; i++){
...
#ifdef DEBUG_PID
    static unsigned int idps = 0, idms = 0;
    if (!(0x1<<id_p&idps) && (id_p!=2 || lv_kp->Vect().Mag()>22)) {
      idps |= 0x1<<id_p;
      printf("id_p = %d (%.1f GeV): ",id_p,lv_kp->Vect().Mag());
      for (int ilh = 0; ilh<5; ilh++)
	printf("%d: %.4f ",ilh,pp_lh2[ilh]/pp_lh2[5]);
      printf("\n");
    }
    if (!(0x1<<id_m&idms) && (id_m!=2 || lv_km->Vect().Mag()>22)) {
      idms |= 0x1<<id_m;
      printf("id_m = %d (%.1f GeV): ",id_m,lv_km->Vect().Mag());
      for (int ilh = 0; ilh<5; ilh++)
	printf("%d: %.4f ",ilh,pm_lh2[ilh]/pm_lh2[5]);
      printf("\n");
    }
#endif
    ...
    if( pipe || !rpipe){
      ...
      bool selectKm = p_bin_m!=-1 && t_bin_m!=-1; if (selectKm) {
#ifdef LOOSE_K_SELECTION
	if (id_p==0 ||                                  // pi
	    lv_kp->Vect().Mag()>p_thr2*1.1 && id_p==2 ||// p above threshold
	    lv_kp->Vect().Mag()>piKMx ||                // Above piK resolution
	    lv_kp->Vect().Mag()<pieMx &&                // e below pie resolution
	    pp_lh2[3]>2*pp_lh2[5] && pp_lh2[3]>1.5*pp_lh2[1]) 
	  selectKm = false;
#else
...
#ifdef DEBUG_IPHI
	if (verbose) printf("%d,%d %.2f\n",Run,(int)Evt,lv_phi->Mag());
#endif
	...
	// 				}
      ...
      bool selectKp = p_bin_p!=-1 && t_bin_p!=-1; if (selectKp) {
#ifdef LOOSE_K_SELECTION
	if (id_m==0 ||                                  // pi
	    lv_km->Vect().Mag()>p_thr2*1.1 && id_m==2 ||// p above threshold
	    lv_km->Vect().Mag()>piKMx ||                // Above piK resolution
	    lv_km->Vect().Mag()<pieMx &&                // e below pie resolution
	    pm_lh2[3]>2*pm_lh2[5] && pm_lh2[3]>1.5*pm_lh2[1])
	  selectKp = false;
#else
...
void get_plots(){
  ...
  if (!input_phi || input_phi->IsZombie()) {
    printf("** fit_table: Error opening TFile \"%s\"\n",hist_file_phi.c_str());
    abort();
  }
  for(int i = 2; i<4; i++){      //6
...
void fit_table_K0(int cc){
  ...
  for(int t = 0; t<Nt; t++){
	...
	} while (r[0+cc][p][t]->covQual()!=3 && iter<=retry);
	printf("=> %d iters -> %d\n",iter,r[0+cc][p][t]->covQual());
      }else {
	...
	frames[iF]->GetXaxis()->SetTitleOffset(1.01);
	frames[iF]->GetXaxis()->SetTitleSize(0.045);
	frames[iF]->GetXaxis()->SetLabelSize(0.045);
	frames[iF]->GetXaxis()->SetNdivisions(505);
	frames[iF]->GetYaxis()->SetTitleOffset(1.3);
	frames[iF]->GetYaxis()->SetTitleSize(0.045);
	frames[iF]->GetYaxis()->SetLabelSize(0.045);
	frames[iF]->GetYaxis()->SetNdivisions(505);
	frames[iF]->SetStats(0);
      }
      c->Clear();
      TPad* pa[5];
	...
	pa[dr]->Draw();
      ...
      static TFile *fout = 0; static TDirectory *rootOut;
      TDirectory *rootApp = gDirectory;
      char cN[] = "cpP99T99", mp[] = "mp"; sprintf(cN,"c%cP%dT%d",mp[cc],p,t);
      c->SetName(cN);
      if(p==0 && t == 0){
	if      (cc == 0) {
	  c->Print("test_K0_0.pdf(");
	  fout = TFile::Open("test_K0_0.root","RECREATE");
	}
	else if (cc == 1) {
	  c->Print("test_K0_1.pdf(");
	  fout = TFile::Open("test_K0_1.root","RECREATE");
	}
	c->Write(cN); rootOut = gDirectory;
      }
      else if(p==Np-1 && t == Nt-1){
	if      (cc == 0) c->Print("test_K0_0.pdf)");
	else if (cc == 1) c->Print("test_K0_1.pdf)");
	rootOut->cd(); c->Write(cN); fout->Close();
      }
      else {
	if      (cc == 0) c->Print("test_K0_0.pdf");
	else if (cc == 1) c->Print("test_K0_1.pdf");
	rootOut->cd(); c->Write(cN);
      }
      delete c; rootApp->cd();
    }
  }
...
void fit_table_phi(int cc){
  ...
  for(int t = 0; t<Nt; t++){
    for(int p = 0; p<Np; p++){
      ...
      int last; if (t==Nt-1) { // Last theta bin: skip last two p bins 
	if      (p==6) last = 1;
	...
	pa[dr]->Draw();
      ...
      if(!fout){
	if      (cc == 0) {
	  c->Print("test_iphi_0.pdf(");
	  fout = TFile::Open("test_iphi_0.root","RECREATE");
	}
	else if (cc == 1) {
	  c->Print("test_iphi_1.pdf(");
	  fout = TFile::Open("test_iphi_1.root","RECREATE");
	}
	c->Write(cN); rootOut = gDirectory;
      }
      else if (last) {
	if      (cc == 0) c->Print("test_iphi_0.pdf)");
	else if (cc == 1) c->Print("test_iphi_1.pdf)");
	rootOut->cd(); c->Write(cN); fout->Close(); fout = 0;
      }
      else {
	if      (cc == 0) c->Print("test_iphi_0.pdf");
	else if (cc == 1) c->Print("test_iphi_1.pdf");
...
void fit_table_Lambda(int cc){
  ...
  for(int t = 0; t<Nt; t++){
	...
	int iter = 0; do {
	  ...
	  iter++;
	} while (r[4+cc][p][t]->covQual()!=3 && iter<=retry);
	printf("=> %d iters -> %d\n",iter,r[4+cc][p][t]->covQual());
      }else {
	...
	pa[dr]->Draw();
      ...
      if(p==0 && t == 0){
	if       (cc == 0) {
	  c->Print("test_Lambda_0.pdf(");
	  fout = TFile::Open("test_Lambda_0.root","RECREATE");
	}
	else if  (cc == 1) {
	  c->Print("test_Lambda_1.pdf(");
	  fout = TFile::Open("test_Lambda_1.root","RECREATE");
	}
	c->Write(cN); rootOut = gDirectory;
      }
      else if(p==Np-1 && t == Nt-1){
...
void print_table(){
  ...
  for(int i = start; i<stop; i++) {  // particle (pi,k,p) 0,6
	...
	int ipt, p; for(p=ipt = 0; p< Np; p++){
	  ...
	  //#define DEBUG_PAR_INDEX
#ifdef DEBUG_PAR_INDEX
	  // Check that the indices of the parameters of relevance are
	  // indeed those listed in "cov_elem".
	  // Ideally, we would need to get "cov_elem" to be derived from the
	  // RooFitResult object, as sketched below.
	  static unsigned int is = 0;
	  if (!(0x1<<i&is)) {
	    is |= 0x1<<i;
	    printf("==== Arg. list for channel \"%s\":",chan[i].c_str());
	    const RooArgList &args = r[i][p][t]->floatParsFinal();
	    const char *parNames[] = {"N_pi_s","N_k_s","N_p_s","N_u_s"};
	    int nPars = sizeof(parNames)/sizeof(char*);
	    for (int ipar = 0; ipar<(int)nPars; ipar++) {
	      const char *name = parNames[ipar];
	      printf(" %d = \"%s\"",args.index(name),name);
	    }
	    printf("\n");
	  }
#endif	  
